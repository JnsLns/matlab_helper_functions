...
function angleToPointCell = trajAngleToPointVersusTgt_WIP(trajData,refPos,tgtPos,coordCols,outCol)
%
...
for curTraj = 1:numel(trajData)
    ...
    for curRow = 1:size(tr,1)-1
        ...
        tgtVector = tgtPos(curTraj,:) - tr(curRow,coordCols);        
                
        % unsigned angle to ref item
        ...
        % unsigned angle to tgt item
        cos_angle = dot(tgtVector,movementVector)/ sqrt(sum(tgtVector.^2)*sum(movementVector.^2));                 
        if 1 < abs(cos_angle) % prevent rounding errors that make cosine slightly larger than one
            cos_angle = sign(cos_angle);
        end
        angleToTgt(curRow,1) = acos(cos_angle);
        
    end
    
    dummy = 0;
    if strcmp(outCol,'single')
        trajData{curTraj} = [angleToPoint-angleToTgt;dummy];
    else        
        trajData{curTraj}(:,outCol) = [angleToPoint-angleToTgt;dummy];
    end
